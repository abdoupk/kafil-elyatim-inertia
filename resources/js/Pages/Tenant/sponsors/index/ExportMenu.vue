...
const printPdf = () => {
    print({
        printable: route('tenant.sponsors.export.pdf', props.params),
        type: 'pdf',
        onLoadingStart: () => {
...
<template>
    <base-menu>
        ...
        <base-menu-items class="w-48" placement="bottom-start">
            ...
            <base-menu-item as="a" :href="route('tenant.sponsors.export.xlsx', params)">
                <svg-loader name="icon-file-excel" class="me-2 h-4 w-4 fill-current" />
                {{ $t('export', { type: 'excel' }) }}
            </base-menu-item>
            <base-menu-item as="a" :href="route('tenant.sponsors.export.pdf', params)">
