...
import DataTable from '@/Pages/Tenant/financials/DataTable.vue'
...
const props = defineProps<{
    finances: PaginationData<FinancesIndexResource>
    params: IndexParams
}>()
...
const deleteFamily = () => {
    router.delete(route('tenant.finances.destroy', selectedFamilyId.value), {
        preserveScroll: true,
        onStart: () => {
...
const showDeleteModal = (financeId: string) => {
    selectedFamilyId.value = financeId
...
<template>
    ...
    <template v-if="finances.data.length > 0">
        ...
        <pagination-data-table
            v-if="finances.meta.last_page > 1"
