...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('orphans', function (Blueprint $table) {
            $table->foreign(['created_by'], 'orphans_created_by_fkey')->references(['id'])->on('users')->onUpdate('no action')->onDelete('no action');
            $table->foreign(['deleted_by'], 'orphans_deleted_by_fkey')->references(['id'])->on('users')->onUpdate('no action')->onDelete('no action');
